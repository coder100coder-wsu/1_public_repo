...
df_merged['jobtype_2'] = np.select(condlist=[df_merged['jobtype'] != "flexible schedule job"
                                             ],
                                   choicelist=[np.nan],
                                   default=np.nan)
...
df_merged['is_fully_remote'] = np.select(condlist=[df_merged['jobtype'] == "100% remote job",
                                                   ...
                                                   df_merged[
                                                       'jobtype'] == "remote - during pandemic job"
                                                   ],
...
df_merged['sal_global_multiplier'] = df_merged['hourly_multiplier'] + df_merged[
    'monthly_multiplier'] + df_merged['yearly_multiplier']
...
split_location['city_clean'] = np.where(split_location['city'] == "us national", "remote",
                                        split_location['city'])
split_location['city_clean'] = np.where(split_location['city_clean'].str.contains('remote'),
                                        "remote", split_location['city_clean'])
...
print(
    "only {}% jobtitles have either of these keywords 'data_analyst','data_scientist','data_engineer'.".format(
        a))
...
df_merged_clean_subset = df_merged_clean[
    ['sal_max_yearly', 'sal_min_yearly', 'is_fully_remote', 'is_partial_remote']]
...
median_df_grouped_salary_max = pd.DataFrame(
    df_merged_clean_subset.groupby(groupby_column_name)['sal_max_yearly'].agg('median'))
...
For other age_groups, generally covid_19 for sex=Female is lower than sex=Male."""
